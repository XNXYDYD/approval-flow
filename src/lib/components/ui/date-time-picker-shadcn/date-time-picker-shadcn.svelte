<script lang="ts">
  ...
  function handleConfirm() {
    ...
    console.log(
      '[DateTimePickerShadcn] handleConfirm - selectedDate:',
      selectedDate,
      'tempTime:',
      tempTime,
      'ISO:',
...
<div bind:this={containerEl} class={cn('relative inline-block w-full', className)}>
  <!-- 触发按钮 -->
  <Button
    {id}
    variant="outline"
  ...
  <!-- 弹出内容 -->
  {#if open}
    <div class="absolute z-50 mt-2 rounded-md border bg-popover p-0 shadow-md outline-none">
      <div class="flex">
        <!-- 左侧日历 -->
        <div class="p-3">
        ...
        <div class="flex flex-col border-l border-border" style="width: 180px;">
          <div class="p-3 space-y-3">
            <div class="space-y-1">
              ...
              <TimePickerInput value={tempTime} on:change={handleTimeChange} />
            </div>
      ...
      <div class="flex items-center justify-between border-t border-border px-3 py-2">
        ...
        <div class="flex items-center gap-2">
          <Button variant="ghost" size="sm" type="button" on:click={handleClear}>清空</Button>
          <Button size="sm" type="button" on:click={handleConfirm} disabled={!selectedDate}>
            确定
          </Button>
        </div>
      </div>
