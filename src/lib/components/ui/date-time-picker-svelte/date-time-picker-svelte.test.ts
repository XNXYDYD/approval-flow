...
describe('DateTimePickerSvelte', () => {
    ...
    it('点击确认按钮（无选择时）应该派发 change 事件', async () => {
      ...
      await vi.waitFor(() => {
        const confirmBtn = Array.from(container.querySelectorAll('button'))
          .find(btn => btn.textContent?.includes('确认'));
        return !!confirmBtn;
      });
  ...
  describe('多步操作', () => {
    it('应该支持连续打开关闭选择器', async () => {
      ...
      let confirmBtn = Array.from(container.querySelectorAll('button'))
        .find(btn => btn.textContent?.includes('确认'));
      await fireEvent.click(confirmBtn!);
      ...
      await vi.waitFor(() => {
        confirmBtn = Array.from(container.querySelectorAll('button'))
          .find(btn => btn.textContent?.includes('确认'));
        return !!confirmBtn;
      });
  ...
  describe('四种核心场景', () => {
    ...
    it('场景2：选了日期 + 不改时间 → 确认，使用选中的日期', async () => {
      ...
      // 模拟用户点击一个日期单元格（非今天）
      const cells = container.querySelectorAll('.cell');
      const nonTodayCells = Array.from(cells).filter(cell => !cell.classList.contains('disabled'));
      const targetCell = nonTodayCells.find(cell => !cell.classList.contains('today')) || nonTodayCells[0];
